...
import com.jfinal.weixin.util.KuWoUtil;
...
public class WeixinMsgController extends MsgControllerAdapter {
    ...
    protected void processInTextMsg(InTextMsg inTextMsg) {
        ...
        if (userMap.get(inTextMsg.getFromUserName()) != null) {
            ...
            user = new User();
            user.setUserId(inTextMsg.getFromUserName());
            user.setMusicPage(1);
            userMap.put(inTextMsg.getFromUserName(), user);
        }


        //用户发来的消息
        String userMsg = inTextMsg.getContent();
        //去掉前后的空格，并将中文感叹号替换为英文感叹号
        userMsg = userMsg.trim().replace("！", "!");


        if ("帮助".equals(userMsg)) {
            outMsg.setContent(
                "输入：\n" +
                "#音乐! + 歌手或者歌曲名\t可以获取歌曲列表\n" +
                "例如：#音乐! 王菲\n" +
        ...
        //以 #电影!  开头，并且不是以  !  结束，证明后面有数据
        if (userMsg.startsWith("#电影!") && !userMsg.endsWith("!")) {
            //获取!后面的数据
            String searchMovieName = userMsg.substring(userMsg.indexOf("!") + 1).trim();

            //保存数据
//            user.setSearchName(searchMovieName);
//            userMap.put(inTextMsg.getFromUserName(), user);

            String movieStr = MovieUtil.searchMovie(searchMovieName);

            if ("-1".equals(movieStr)){
                outMsg.setContent("没有找到此电影");
                render(outMsg);
                return;
            }
            outMsg.setContent(movieStr);
            render(outMsg);
            return;
            //看电影 !xxxx
        } else if (userMsg.startsWith("!")){
            //截取后面
            String movieStr = userMsg.substring(1).trim();


            News news = new News();
            news.setTitle("观看电影");
            news.setUrl("https://439f1k6694.oicp.vip/movie?vodId=" + movieStr );
            ...
            //验证是不是以"#音乐!"开头，并且不是以”！“结尾
            if (userMsg.startsWith("#音乐!") && !userMsg.endsWith("!")) {
                //截取用户需要查询的歌曲或歌手
                String searchName = userMsg.substring(userMsg.indexOf("!") + 1).trim();
                ...
                user.setMusicPage(1);
                //获取歌曲列表
                String musicStr = KuWoUtil.searchMusicByPage(searchName, user.getMusicPage());

                musicStr += "@# 代表下一页";

                //返回给用户信息
                outMsg.setContent(musicStr);
                render(outMsg);
                //下一页情况
                //@#：下一页
                //@！：上一页
                ...
                //截取@之后的数据
                String userStr = userMsg.substring(1);
                //用户只输入了一个@
                if (userStr.length() == 0) {
                    outMsg.setContent("格式错误，输入”帮助“获取指令");
                    render(outMsg);
                    return;
                }
                switch (userStr) {
                    ...
                    case "#":
                        //保存页码
                        user.setMusicPage(user.getMusicPage() + 1);
                        userMap.put(user.getUserId(), user);
                        //获取下一页数据
                        String nextPageData = KuWoUtil.handlePage(KuWoUtil.searchMusicByPage(user.getSearchName(), user.getMusicPage()), user.getMusicPage());
                        //返回给用户信息
                        outMsg.setContent(nextPageData);
                        render(outMsg);
                        break;
                    //@! 下一页  英文状态和中文状态的感叹号
                    case "!":
                    case "！":
                        ...
                        user.setMusicPage(user.getMusicPage() - 1);
                        userMap.put(user.getUserId(), user);
                        String previousPageData = KuWoUtil.handlePage(KuWoUtil.searchMusicByPage(user.getSearchName(), user.getMusicPage()), user.getMusicPage());
                        //返回给用户信息
                        outMsg.setContent(previousPageData);
                        render(outMsg);
                        break;
                    ...
                    default:
                        //验证是否是纯数字
                        //不是纯数字，即代表不是找歌
                        if (!KuWoUtil.isNumber(userStr)) {
                            outMsg.setContent("格式错误，输入”帮助“获取指令");
                            render(outMsg);
                            return;
                        }
                        String mp3Url = KuWoUtil.getMP3Url(userStr);
                        if ("".equals(mp3Url)) {
                            outMsg.setContent("没有找到");
                            render(outMsg);
                            return;
                        }

                        OutMusicMsg musicMsg = new OutMusicMsg(inTextMsg);
                        musicMsg.setHqMusicUrl(mp3Url);
                        musicMsg.setTitle("开始播放");
                        render(musicMsg);
                        break;
                }
